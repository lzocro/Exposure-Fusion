...
def Reduce(image):
    ...
    Input : np.array with 2 axis, test with 3 axis
    Output : reduced dims np.array with 2 axis
    """
    # convolution step with kernel
    convol_image = cv2.filter2D(image, cv2.CV_32FC3, K, borderType=cv2.BORDER_REFLECT_101) # half-symmetric boundary extension
    
    # resizing step (keeping even columns)
    if len(convol_image.shape) == 2:
        convol_image = np.expand_dims(convol_image, axis=2)
    new_image = convol_image[::2,::2,:]
#    new_image = cv2.resize(convol_image, None, fx=0.5, fy=0.5)
...
def Expand(image,pad_h=0, pad_w=0):
    ...
    Output : extended dims np.array with 2 axis, test with 3 axis
    """
    im_border = cv2.copyMakeBorder(image,1,1,1,1, borderType=cv2.BORDER_REPLICATE)
    new_image = np.zeros((2*im_border.shape[0],2*im_border.shape[1], im_border.shape[2]), dtype=np.float32)
    new_image[::2,::2,:] = 4*im_border
    convol_image = cv2.filter2D(new_image,cv2.CV_32FC3,K,borderType=cv2.BORDER_REFLECT_101)
...
def Gaussian_pyramid(image, depth=None):
    ...
    Image values are btw [0,1]
    Input : np.array with two axis test with 3 axis
    Output : list of np.arrays with two axis, len=maximal depth test with 3 axis
    """
    gaus_pyr = []
    gaus_pyr.append(image)
    i = 0
    while min(gaus_pyr[-1].shape[:2])>1:
...
def Laplacian_pyramid(image, depth=None):
    ...
    Output : list of np.arrays with two dims test with 3 axis
    """
    lapl_pyr = []
    ...
    for i in range(depth-1,-1,-1):
        if i==depth-1:
            ...
            print(gaus_pyr[i].shape)
            print(Expand(gaus_pyr[i+1], pad_h, pad_w).shape)
...
def Collapse_Laplacian(laplacian_pyr):
    ...
    Output : np.array with 2 axis test with 3 axis
    """
    u = laplacian_pyr[0]
